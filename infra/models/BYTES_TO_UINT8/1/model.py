            ...
            if bytes_t is None or size_t is None:
                responses.append(
                    pb_utils.InferenceResponse(
                        error=pb_utils.TritonError("Missing BYTES or IMG_SIZE")
                    )
                )
                continue
            try:
                ...
                if size.size != 2:
                    raise ValueError(
                        "IMG_SIZE must be a 2-element int32 tensor: [H, W]"
                    )
                h = int(size[0])
                w = int(size[1])
                if h <= 0 or w <= 0:
                ...
                out2 = pb_utils.Tensor(
                    "IMG_ORIGINAL", np.frombuffer(raw, dtype=np.uint8)
                )
                responses.append(
                    pb_utils.InferenceResponse(output_tensors=[out1, out2])
                )
