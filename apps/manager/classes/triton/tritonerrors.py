...
class TritonModelNotReady(Exception):
    def __init__(self, model_name: str, timeout: int):
        ...
        self.timeout = timeout
        super().__init__(
            f"{STATIC_START}Model '{model_name}' failed to become ready within {timeout}s"
        )
...
class TritonServerStateChanged(Exception):
    def __init__(self, vm_ip: str, container_id: str, changed_fields: list):
        ...
        super().__init__(
            f"{STATIC_START}Server ({vm_ip}, {container_id[:12]}) state changed: {changed_fields}"
        )
...
class TritonServerCreationFailed(Exception):
    def __init__(self, vm_ip: str, container_id: str, reason: str):
        ...
        self.reason = reason
        super().__init__(
            f"{STATIC_START}Server creation failed for ({vm_ip}, {container_id[:12]}): {reason}"
        )
...
class TritonMissingInstance(Exception):
    def __init__(self, vm_id: str, container_id: str):
        ...
        self.container_id = container_id
        super().__init__(
            f"{STATIC_START}Server instance not found: vm_id='{vm_id}', container_id='{container_id}'"
        )
