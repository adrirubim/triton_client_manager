...
from .info.data import VM, Flavor, Host, Image, Keypair, Network, Security
...
class OpenstackThread(threading.Thread):
    def __init__(self, **kwargs):
        ...
        self.openstack_creation = OpenstackCreation(
            self.auth, kwargs.get("creation_timeout"), kwargs.get("creation_endpoint")
        )
        self.openstack_deletion = OpenstackDeletion(
            self.auth, kwargs.get("deletion_timeout"), kwargs.get("deletion_endpoint")
        )
    ...
    def create_vm(self, data: dict) -> str:
        ...
        # --- Not found ---
        with self._data_lock:
            if data["keypair"] not in self.dict_keypairs:
                raise OpenstackResourceNotFound("keypair", data["keypair"])
            if data["image_id"] not in self.dict_images:
                raise OpenstackResourceNotFound("image_id", data["image_id"])
            if data["security"] not in self.dict_securities:
                raise OpenstackResourceNotFound("security", data["security"])
            if data["flavor_id"] not in self.dict_flavors:
                raise OpenstackResourceNotFound("flavor_id", data["flavor_id"])
            if data["network_id"] not in self.dict_networks:
                raise OpenstackResourceNotFound("network_id", data["network_id"])
