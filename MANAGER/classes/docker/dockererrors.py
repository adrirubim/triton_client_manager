...
class DockerContainerStateChanged(Exception):
    def __init__(self, container_id: str, container_name: str, worker_ip: str, changed_fields: list[str]):
        self.container_id = container_id
        self.container_name = container_name
        self.worker_ip = worker_ip
        self.changed_fields = changed_fields
        super().__init__(f"{STATIC_START}Container '{container_name}' ({container_id[:12]}) on {worker_ip} state changed: {', '.join(changed_fields)}")
