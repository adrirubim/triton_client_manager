...
class DockerDeletion:
    ...
    def handle(self,
        ...
        try:
            # --- Connect ---
            client = docker.DockerClient(base_url = f'tcp://{ip}:{self.remote_api_port}',
            ...
            if container.status == 'running':
                ...
                if force: container.kill()
                else:     container.stop(timeout=10)
        ...
        except NotFound as e:  raise DockerDeletionError(e)
        except APIError as e:  raise DockerDeletionError(e)
        except Exception as e: raise DockerDeletionError(e)
