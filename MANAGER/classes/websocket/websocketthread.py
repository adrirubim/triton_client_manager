...
class WebSocketThread(threading.Thread):
    ...
    def _validate_message(self, message: dict) -> tuple[bool, str]:
        ...
        if "uuid" not in message:    return False, "Missing required field: 'uuid'"
        if "type" not in message:    return False, "Missing required field: 'type'"
        if "payload" not in message: return False, "Missing required field: 'payload'"
        
        # Validate field types
        if not isinstance(message["uuid"], str): return False, "Field 'uuid' must be a string"
        if not isinstance(message["type"], str): return False, "Field 'type' must be a string"
        if not isinstance(message["payload"], dict): return False, "Field 'payload' must be a dictionary"
        
        # Validate type is in allowed list
        if message["type"] not in self.valid_types:
            return False, f"Invalid type '{message['type']}'. Must be one of: {sorted(self.valid_types)}"
        ...
        try:
            ...
            while True:
                ...
                if message["uuid"] != client_id:
                    await self._send_error(websocket, f"UUID mismatch. Expected '{client_id}', got '{message['uuid']}'")
                    continue  # Don't close connection, just skip this message
