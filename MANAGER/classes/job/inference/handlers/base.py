...
def validate_fields(payload: dict) -> tuple:
    ...
    if not vm_ip:        raise ValueError("Missing required field 'vm_ip'")
    if not container_id: raise ValueError("Missing required field 'container_id'")
    if not model_name:   raise ValueError("Missing required field 'model_name'")
    if not inputs:       raise ValueError("Missing required field 'request.inputs'")
