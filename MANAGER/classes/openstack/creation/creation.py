...
class OpenstackCreation:
    ...
    def handle(self,
        ...
        # --- Send request ---
        response = requests.post(url     = full_endpoint,
                                 json    = payload,
    ...
    def loop_status(self, vm_id: str) -> str:
        ...
        while (time.time() - start_time) < self.timeout:
            try:
                ...
                if vm_status == "ERROR" : raise Exception("VM Creation status in ->> ERROR <<-")
                if vm_status == "ACTIVE": break
