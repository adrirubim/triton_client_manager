...
class OpenstackAuth:
    ...
    def __init__(self, auth_url: str = None, application_credential_id: str = None,
                 application_credential_secret: str = None, region_name: str = "RegionOne",
                 verify_ssl: Union[bool, str] = True, token_refresh_buffer_minutes: int = 10, **kwargs):
        """
        ...
        if not auth_url or not application_credential_id or not application_credential_secret:
            raise ValueError("auth_url, application_credential_id, and application_credential_secret are required")
        
        # Normalize auth URL - handle both base URL and full endpoint
        auth_url = auth_url.rstrip('/')
        if '/v3/auth/tokens' in auth_url: self.auth_url = auth_url  # Full endpoint provided
        else:                             self.auth_url = f"{auth_url}/v3/auth/tokens"  # Base URL, append endpoint
    ...
    def authenticate(self) -> bool:
        ...
        with self.lock:
            try:
                ...
                response = requests.post(url=self.auth_url, json=auth_payload,
                                       headers={"Accept": "application/json"},
                                       verify=verify_param, timeout=30)
                ...
                logger.info("Authenticated | Expires: %s | Refresh: %s", self.token_expires_at, self.token_refresh_at)
                return True
