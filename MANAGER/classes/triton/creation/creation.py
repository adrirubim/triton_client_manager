...
class TritonCreation:
    ...
    def handle(self,
               ...
               minio: dict,
               triton: dict,
               container_id: str) -> TritonServer:

        # --- Extrapolate config.pbtxt ---
        config_json, inputs, outputs, model_name, port = self._process_config(minio_config = minio,
                                                                              triton_params = triton)
        # --- Check ---
        if not model_name: raise TritonModelLoadFailed("unknown", "Could not determine model name from config.pbtxt")
        if not inputs:     raise TritonModelLoadFailed(model_name, "config.pbtxt has no inputs")
        if not outputs:    raise TritonModelLoadFailed(model_name, "config.pbtxt has no outputs")

        # --- Single client based on port ---
        if port == GRPC_PORT: client = grpcclient.InferenceServerClient(url=f"{vm_ip}:{GRPC_PORT}")
        else:                 client = httpclient.InferenceServerClient(url=f"{vm_ip}:{HTTP_PORT}")

        # --- Wait server ready ---
        start = time.time()
        while (time.time() - start) < self.server_ready_timeout:
            time.sleep(1)
            if client.is_server_ready(client_timeout=self.client_request_timeout): break
        else:
            raise TritonServerHealthFailed(self.server_ready_timeout)

        # --- Wait for request loading ---
        try: client.load_model(model_name, config=config_json, client_timeout=self.client_request_timeout)
        except Exception: raise TritonModelLoadFailed(model_name, "Load request failed")

        # --- Wait for model to be loaded ---
        start = time.time()
        while (time.time() - start) < self.model_ready_timeout:
            time.sleep(1)
            if client.is_model_ready(model_name, client_timeout=self.client_request_timeout): break
        else:
            raise TritonModelNotReady(model_name, self.model_ready_timeout)
        
        
        # --- Create Server ---
        server = TritonServer(vm_id        = vm_id,
                              vm_ip        = vm_ip,
                              status       = "ready",
    ...
    def _download_pbtxt(self,
                        key: str,
                        minio_config: dict) -> str:
        try:
            s3 = boto3.client(
                's3',
    ...
    def _pbtxt_to_config(self,
                         parameters: dict,
                         pbtxt_content: str) -> tuple:
    ...
    def _process_config(self, minio_config: dict, triton_params: dict) -> tuple:
        ...
        logger.info(" Downloading config from {minio_config['endpoint']}/{minio_config['bucket']}/{key}")
