...
class ClientManager():
    ...
    def config(self):
        ...
        with open("config/jobs.yaml",      encoding="utf-8") as f: self.config_job       = safe_load(f)
        with open("config/docker.yaml",    encoding="utf-8") as f: self.config_docker    = safe_load(f)
        with open("config/openstack.yaml", encoding="utf-8") as f: self.config_openstack = safe_load(f)
        with open("config/websocket.yaml", encoding="utf-8") as f: self.config_websocket = safe_load(f)
        with open("config/triton.yaml",    encoding="utf-8") as f: self.config_triton    = safe_load(f)
    ...
    def setup(self):
        ...
        if not self.openstack.wait_until_ready(timeout=30): raise TimeoutError("OpenStack thread failed to initialize")
        logger.info("[ClientManager] ✓ OpenStack ready")

        logger.info("[ClientManager] Starting Triton thread...")
        self.triton.start()
        if not self.triton.wait_until_ready(timeout=30): raise TimeoutError("Triton thread failed to initialize")
        logger.info("[ClientManager] ✓ Triton ready")

        logger.info("[ClientManager] Starting Docker thread...")
        self.docker.start()
        if not self.docker.wait_until_ready(timeout=30): raise TimeoutError("Docker thread failed to initialize")
        logger.info("[ClientManager] ✓ Docker ready")

        logger.info("[ClientManager] Starting Job thread...")
        self.job.start()
        if not self.job.wait_until_ready(timeout=30): raise TimeoutError("Job thread failed to initialize")
        logger.info("[ClientManager] ✓ Job ready")

        logger.info("[ClientManager] Starting WebSocket thread...")
        self.websocket.start()
        if not self.websocket.wait_until_ready(timeout=30): raise TimeoutError("WebSocket thread failed to initialize")
